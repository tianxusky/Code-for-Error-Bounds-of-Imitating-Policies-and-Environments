...
def collect_samples_from_true_env(env, actor, nb_episode=50, seed=2020):
...
def main():
    ...
    expert_state, expert_action, expert_next_state, expert_reward = collect_samples_from_true_env(
        env=env, actor=actor, nb_episode=FLAGS.GAIL.traj_limit)
    ...
    for n_updates in range(FLAGS.GAIL.pretrain_iters):
        indices = np.random.randint(low=0, high=len(expert_state), size=FLAGS.BC.batch_size)
        ...
        if n_updates % 100 == 0:
            mse_loss = policy.get_mse_loss(expert_state_, expert_action_, expert_next_state_)
    ...
    for t in range(0, FLAGS.GAIL.total_timesteps, FLAGS.TRPO.rollout_samples*FLAGS.GAIL.g_iters):
        ...
        for n_update in range(FLAGS.GAIL.g_iters):
            ...
            indices = np.random.randint(low=0, high=len(expert_state), size=256)
            expert_state_ = expert_state[indices]
            expert_action_ = expert_action[indices]
            expert_next_state_ = expert_next_state[indices]
        ...
        for n_update in range(FLAGS.GAIL.d_iters):
            ...
            for generator_subset in generator_dataset.iterator(batch_size):
                ...
                train_info = discriminator.train(
                    expert_state_, expert_action_, expert_next_state_,
